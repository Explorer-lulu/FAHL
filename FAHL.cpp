...
void FRNGraph::FAHLconOrderMT(string orderfile){
...
void FRNGraph::FAHLdecBat(vector<pair<pair<int,int>,pair<int,int>>>& wBatch){
...
int FRNGraph::QueryFAHL(int ID1,int ID2){
	if(ID1==ID2) return 0;
...
vector<int> FRNGraph::PathfindFAHL(int ID1, int ID2){
	vector<int> Path;
	...
	if(LCA==r1){
		Path=SubPathfindFAHL(ID2,ID1);
	}
	else if(LCA==r2){
		Path=SubPathfindFAHL(ID1,ID2);
	}
	else{
		for(int i=0;i<Tree[LCA].pos.size();i++){
		...
		Path=SubPathfindFAHL(ID1,LCAonSP);

		vector<int> path1=SubPathfindFAHL(ID2, LCAonSP);
		for(int k=path1.size()-2;k>=0;k--){
			Path.push_back(path1[k]);
		}
	}
	cout<<"Find Path With "<<Path.size()<<endl;
	return Path;
}

vector<int> FRNGraph::SubPathfindFAHL(int ID1, int ID2){
	vector<int> Path;
	if(ID1==ID2) return Path;
	if(NodeOrder[ID1]==-1 || NodeOrder[ID2]==-1) return Path;
...
void FRNGraph::FAHLincBatMT(vector<pair<pair<int,int>,pair<int,int>>>& wBatch){
	int checknum=0;
